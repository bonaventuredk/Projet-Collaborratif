...
def B_N_spires(x, y, z,I, R_coil, nb_spires,spacing ):
    ...
    for z0 in z_centers:  # Pour chaque spire
        bx, by, bz = B_spire_3D(x, y, z, I, R_coil, z0)  # Champ de cette spire
...
def simulate_until_exit(X0, U0, charge_sign, dt,ux_interp=None, uy_interp=None, bobine= None,model = None, max_steps=100):
    ...
    while X[-1][0] < L and i < max_steps:
        ...
        # Champ magnétique
        x_coil = bobine["x_coil"]
        y_coil = bobine["y_coil"]
        z_coil = bobine["z_coil"]
        I = bobine["I"]
        R_coil = bobine["Rayon"]
        nb_spires = bobine["Nb_spire"]
        spacing = bobine["spacing"]
        B0 = bobine["B0"] 
...
def run_multi_lap(n_particles_by_charge, total_laps, dt, bobine, verbose=True):
    """
