...
def run_multi_lap(n_particles_by_charge, total_laps, dt, bobine,
    ...
    if preloaded_interps is not None:
        ...
        nodes_file = f"nodes{L_loc}.txt"
        ux_file    = f"ux{L_loc}.txt"
        uy_file    = f"uy{L_loc}.txt"
...
def draw_domain():
    """Dessine le contour du domaine (murs extérieurs, séparateurs, pointes)."""
    n     = 1.5
    alpha = 2.0 / n
    Rtip_d = eta / 2
...
if __name__ == "__main__":
    ...
    # BUG CORRIGÉ #8 : dict bobine construit et transmis correctement
    L_sim = 0.076
    bobine_sim = {
        "L"        : L_sim,
        "I"        : I,
