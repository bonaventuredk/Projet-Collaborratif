...
def damping_y(y, Uy, D, gamma_max=1000.0, delta_zone=4e-4):
    """
    Applique un amortissement artificiel sur la vitesse verticale Uy
    lorsque le point (y) se trouve proche des parois horizontales y = 0
    ou y = D. Cet amortissement permet de stabiliser numériquement le
    champ de vitesse en évitant des oscillations non physiques près des
    frontières.

    Paramètres
    ----------
    y : float
        Position verticale du point considéré.
    Uy : float
        Vitesse verticale locale avant amortissement.
    D : float
        Hauteur totale du domaine.
    gamma_max : float, optionnel
        Intensité maximale de l’amortissement appliqué au contact du mur.
    delta_zone : float, optionnel
        Épaisseur de la zone proche du mur dans laquelle l’amortissement
        augmente linéairement.

    Retour
    ------
    float
        Valeur amortie de la vitesse verticale Uy.

    Notes
    -----
    - L’amortissement est nul dans la zone centrale du domaine.
    - Il croît linéairement lorsqu’on s’approche des murs.
    - Le signe négatif final garantit que l’amortissement agit comme une
      force opposée au mouvement.
    """

    # Valeur initiale de l’amortissement (aucun effet par défaut)
    damping = 0.0

    # Zone proche du mur inférieur (y = 0)
    if y < delta_zone:
        # Amortissement croissant linéairement à mesure que y → 0
        damping = gamma_max * (delta_zone - y) / delta_zone

    # Zone proche du mur supérieur (y = D)
    elif y > D - delta_zone:
        # Amortissement croissant linéairement à mesure que y → D
        damping = gamma_max * (y - (D - delta_zone)) / delta_zone

    # Retourne la vitesse amortie : force opposée au mouvement
    return -damping * Uy
...
def simulate_particle(X0, U0, charge_sign=1):
    ...
    for i in range(Nt-1):
        ...
        U_new[1] += dt * damping_y(X[i,1], U_new[1], D)
...
y_milieu_min = lwall + eta
y_milieu_max = D - lwall - eta
y_rouge_min  = D - lwall
